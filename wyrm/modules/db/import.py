...
def execute(argv, argv0, engine):
    ...
    def import_migrations_from_app(app):
        ...
        migrations = [ (os.path.join(migrations_dir, m), re.sub(r"^[\d_]+","", m)[:-3] ) for m in sorted(os.listdir(migrations_dir)) if not m.startswith('_') and m.endswith('.py') ]
        for path, name in migrations:
            ...
            lib.insert_in_python(dest_file, ["from"], ["# {} module. please dont remove this line".format(app)], in_end=True, ignore_pass=True)
