...
def usage(argv0):
    print(
        "Usage: {} s [-d] [-p|--port N] [-h|--host HOST] [-c|--config YAML] [--pid PID] [-s|--socket SOCKET] [-S|--servers N]".format(
            argv0))
    sys.exit(1)
...
def execute(argv, argv0, engine):
    ...
    optlist, args = getopt.getopt(argv, "dhc:p:s:S:h:",
                                  ["help", "port=", "config=", "socket=", "servers=", "pid=", "host="])
